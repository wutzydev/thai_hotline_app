# 📱 Thai Hotline App (สายด่วน THAILAND)

แอปพลิเคชันรวบรวมเบอร์โทรศัพท์สายด่วนที่สำคัญในประเทศไทย พัฒนาด้วย **Flutter** ออกแบบ UI ให้สวยงาม ใช้งานง่าย และแบ่งหมวดหมู่ชัดเจน พร้อมฟีเจอร์กดโทรออกได้ทันทีเพื่อความสะดวกรวดเร็วในยามฉุกเฉิน

## ✨ ฟีเจอร์หลัก (Features)
- **Splash Screen:** หน้าต้อนรับเข้าสู่แอปพลิเคชันหน่วงเวลา 3 วินาที
- **หมวดหมู่สายด่วน 4 หมวดหลัก:**
  - 🚗 การเดินทาง
  - 🚑 อุบัติเหตุ-เหตุฉุกเฉิน
  - 🏦 ธนาคาร
  - 💧 สาธารณูปโภค
- **One-Tap Calling:** เชื่อมต่อระบบโทรศัพท์ของตัวเครื่อง สามารถกดปุ่มเพื่อโทรออกเบอร์สายด่วนได้ทันที (ใช้งาน `url_launcher`)
- **About UI:** หน้าแสดงข้อมูลส่วนตัวของผู้จัดทำ

## 📸 ภาพตัวอย่างแอปพลิเคชัน (Screenshots)

<div align="center">
<img width="250" alt="localhost_56844_(iPhone 14 Pro Max)" src="https://github.com/user-attachments/assets/84d9a1aa-0af2-477b-a19c-aa0aa370d1fd" />
<img width="250"  alt="localhost_56844_(iPhone 14 Pro Max) (1)" src="https://github.com/user-attachments/assets/3fe3e906-e23c-4d3d-8001-e8ad05bceb9c" />

<img width="250" alt="localhost_56844_(iPhone 14 Pro Max) (2)" src="https://github.com/user-attachments/assets/e5ef4043-791c-405a-9e1f-ea087f8f5715" />
<img width="250" alt="localhost_56844_(iPhone 14 Pro Max) (3)" src="https://github.com/user-attachments/assets/b4647468-4fc6-4bac-a37a-934ab04d436b" />

<img width="250"  alt="localhost_56844_(iPhone 14 Pro Max) (4)" src="https://github.com/user-attachments/assets/b3837de6-906d-42a7-8754-1f286ed54f4e" />

<img width="250"  alt="localhost_56844_(iPhone 14 Pro Max) (5)" src="https://github.com/user-attachments/assets/353664e6-f5c2-43ae-822c-f31c538d6ab8" />

  </div>

## 🛠️ เทคโนโลยีที่ใช้
- **Framework:** Flutter
- **Language:** Dart
- **Package:** url_launcher
